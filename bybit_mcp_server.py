"""
Bybit MCP Server ÔÇö read-only tools per GitHub Copilot
...
# ÔöÇÔöÇ tools ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
async def get_wallet_balance() -> str:
    ...
    lines = [
        f"­ƒÆ░ Equity totale:     {equity:.4f} USDT",
        f"Ô£à Disponibile:       {available:.4f} USDT",
        f"­ƒôè Margine impiegato: {margin_used:.4f} USDT",
        f"­ƒôê Unrealized PnL:    {unrealized_pnl:+.4f} USDT",
    ]
    return "\n".join(lines)
...
async def get_open_positions() -> str:
    ...
    for p in positions:
        ...
        lev         = p.get("leverage", "?")
        sl          = p.get("stopLoss", "ÔÇö")
        tp          = p.get("takeProfit", "ÔÇö")
        emoji = "­ƒôê" if side == "Buy" else "­ƒôë"
        lines.append(
...
async def get_recent_closed_pnl(limit: int = 20) -> str:
    ...
    for t in trades:
        ...
        emoji = "Ô£à" if pnl > 0 else "ÔØî"
        lines.append(f"{emoji} {dt} | {sym} {side} x{qty} | entry {entry} ÔåÆ close {close} | PnL {pnl:+.4f} USDT")

    lines.append(f"\n­ƒôè Totale P&L ({len(trades)} trade): {total_pnl:+.4f} USDT")
    wins = sum(1 for t in trades if _f(t["closedPnl"]) > 0)
    lines.append(f"­ƒÅå Win rate: {wins}/{len(trades)} ({wins/len(trades)*100:.0f}%)")
    return "\n".join(lines)
...
async def get_pnl_summary(days: int = 7) -> str:
    ...
    lines = [
        f"­ƒôà Ultimi {days} giorni ÔÇö {len(trades)} trade",
        f"",
        f"­ƒÆ░ PnL netto totale:  {total_pnl:+.4f} USDT",
        f"­ƒÅå Win rate:          {len(wins)}/{len(pnls)} ({win_rate:.0f}%)",
        f"­ƒôè Profit Factor:     {profit_factor:.2f}",
        f"­ƒôê Avg vincita:       {avg_win:+.4f} USDT",
        f"­ƒôë Avg perdita:       {avg_loss:+.4f} USDT",
        f"­ƒÑç Miglior trade:     {best:+.4f} USDT",
        f"­ƒÆÇ Peggior trade:     {worst:+.4f} USDT",
        f"",
        f"ÔöÇÔöÇ Breakdown per simbolo ÔöÇÔöÇ",
    ]
    for sym, s in sorted_syms:
        wr    = s["wins"] / s["n"] * 100
        emoji = "Ô£à" if s["pnl"] > 0 else "ÔØî"
        lines.append(
...
async def get_risk_exposure() -> str:
    ...
    for p in positions:
        ...
        if sl > 0:
            ...
            bar = "­ƒƒó" if sl_dist_pct > 3 else ("­ƒƒí" if sl_dist_pct > 1 else "­ƒö┤")
            lines.append(
                f"{emoji} {sym} {side.upper()} ├ù {size}\n"
                f"   Entry: {entry} | Mark: {mark} | SL: {sl}\n"
                f"   {bar} Distanza SL: {sl_dist_pct:.2f}%  |  PnL attuale: {upnl:+.4f} USDT\n"
                f"   ÔÜá´©Å  Max ulteriore perdita se SL colpito: -{loss_from_mark:.4f} USDT ({risk_pct:.1f}% equity)"
            )
        else:
            lines.append(
                f"{emoji} {sym} {side.upper()} ├ù {size}\n"
                f"   Entry: {entry} | Mark: {mark} | PnL: {upnl:+.4f} USDT\n"
                f"   ­ƒÜ¿ NESSUN STOP LOSS IMPOSTATO"
            )

    total_risk_pct = total_risk_usdt / equity * 100 if equity > 0 else 0.0
    lines.append(
        f"ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ\n"
        f"­ƒÆ░ Equity: {equity:.4f} USDT\n"
        f"ÔÜá´©Å  Rischio totale aperto: -{total_risk_usdt:.4f} USDT ({total_risk_pct:.1f}% equity)"
    )
    return "\n\n".join(lines)
...
async def _get_latest_deployment(service_id: str) -> dict | None:
    """Ritorna il nodo del deploy pi├╣ recente per un servizio."""
...
async def get_railway_status() -> str:
    ...
    STATUS_EMOJI = {
        "SUCCESS": "Ô£à", "ACTIVE": "Ô£à",
        "CRASHED": "­ƒö┤", "FAILED": "­ƒö┤",
        "DEPLOYING": "­ƒöä", "BUILDING": "­ƒöä",
        "SLEEPING": "­ƒÿ┤", "QUEUED": "ÔÅ│",
        "REMOVED": "­ƒùæ´©Å",
    }

    def _fmt(name: str, dep: dict | None) -> str:
        if dep is None:
            return f"ÔØô {name}: impossibile recuperare stato"
        status  = dep.get("status", "UNKNOWN")
        emoji   = STATUS_EMOJI.get(status, "ÔØô")
        created = dep.get("createdAt", "")[:16].replace("T", " ")
        updated = dep.get("updatedAt", "")[:16].replace("T", " ")
        meta    = dep.get("meta") or {}
        commit  = str(meta.get("commitMessage", "ÔÇö"))[:60] if isinstance(meta, dict) else "ÔÇö"
        sha     = str(meta.get("commitSha", ""))[:7] if isinstance(meta, dict) else ""
        sha_str = f"[{sha}] " if sha else ""
        return (
            f"{emoji} Bot {name} ÔÇö {status}\n"
...
async def get_railway_logs(bot: str = "short", lines: int = 50) -> str:
    ...
    SEV_EMOJI = {"ERROR": "­ƒö┤", "WARNING": "­ƒƒí", "INFO": ""}
    out_lines = [f"­ƒôï Log bot {bot.upper()} ÔÇö ultimi {len(log_entries)} righe (status: {status})\n"]
...
async def get_bot_summary() -> str:
    ...
    return (
        "ÔòÉÔòÉÔòÉ BILANCIO ÔòÉÔòÉÔòÉ\n" + balance_str +
        "\n\nÔòÉÔòÉÔòÉ POSIZIONI APERTE ÔòÉÔòÉÔòÉ\n" + positions_str +
        "\n\nÔòÉÔòÉÔòÉ RISCHIO APERTO ÔòÉÔòÉÔòÉ\n" + risk_str +
        "\n\nÔòÉÔòÉÔòÉ ULTIMI 10 TRADE ÔòÉÔòÉÔòÉ\n" + pnl_str +
        "\n\nÔòÉÔòÉÔòÉ STATISTICHE 7 GIORNI ÔòÉÔòÉÔòÉ\n" + summary_str +
        "\n\nÔòÉÔòÉÔòÉ STATO BOT RAILWAY ÔòÉÔòÉÔòÉ\n" + railway_str
    )
