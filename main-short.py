...
_swing_entry_meta: dict = {}  # symbol → {"tp": support, "sl": sl_nat, "r_dist": sl_dist}
last_exit_time = {}
...
TP1_R = float(os.getenv("TP1_R", "1.5"))             # PARTIAL-TP: chiude 50% a +1.5R (era 2.5R), validato in backtest +13% PnL
...
def record_exit(symbol: str, entry_price: float, exit_price: float, side: str):
    ...
    if pnl_pct < 0:
        ...
        # Blacklist dinamica: N loss consecutive → blocco duro per 24h
        if recent_losses[symbol] >= BLACKLIST_CONSEC_LOSSES:
            until_ts = time.time() + BLACKLIST_HOURS * 3600
            symbol_blacklist_until[symbol] = until_ts
            tlog(f"blacklist:{symbol}",
                 f"[BLACKLIST][SHORT] {symbol} {BLACKLIST_CONSEC_LOSSES} loss consecutive → bloccato per {BLACKLIST_HOURS}h", 3600)
            notify_telegram(f"🚫 [BLACKLIST][SHORT] {symbol} — {BLACKLIST_CONSEC_LOSSES} loss di fila\nBloccato per {BLACKLIST_HOURS}h, riprende alle {__import__('datetime').datetime.fromtimestamp(until_ts).strftime('%H:%M')} UTC")
        ...
        if abs(pnl_pct) >= (MAX_LOSS_CAP_PCT * 0.5 * 100):  # oltre metà del cap = perdita grande
            ...
            tlog(f"big_loss_cd:{symbol}", f"[BIG-LOSS-CD][SHORT] {symbol} perdita grande ({pnl_pct:.1f}%) → cooldown 24h", 3600)
    else:
        recent_losses[symbol] = 0
        last_exit_was_loss[symbol] = False
        # Rimuovi dalla blacklist se torna positivo
        if symbol in symbol_blacklist_until:
            del symbol_blacklist_until[symbol]
            log(f"[BLACKLIST][SHORT] {symbol} rimosso dalla blacklist dopo trade vincente")


# ─────────────────────────────────────────────────────────────────────────────
# Strategia swing-level SHORT: funzioni di supporto (O(n) via numpy precompute)
# ─────────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        if len(df) >= SWING_LOOKBACK + 5:
            try:
                ...
                # Filtro macro: prezzo sotto EMA200 e EMA200 in discesa (downtrend strutturale)
                if (_sw_price <= _sw_ema200 * 1.02
                        and _sw_ema200 <= _sw_ema200p
                        and SWING_RSI_MIN_SHORT <= _sw_rsi <= SWING_RSI_MAX_SHORT
                    ...
                    if _support is not None and _resistance is not None:
                        _dist_resistance = _resistance - _sw_price
                        if (_dist_resistance <= SWING_NEAR_ATR * _sw_atr
                                and _sw_price < _resistance):
                            _sl_nat  = _resistance + SWING_SL_ABOVE_ATR * _sw_atr
                            _sl_dist = _sl_nat - _sw_price
                            if _sl_dist > 0:
                                _rr = (_sw_price - _support) / _sl_dist
                                if _rr >= SWING_RR_MIN:
                                    tlog(f"swing_short:{symbol}",
                                         f"[SWING-LEVEL][SHORT] {symbol} | res={_resistance:.6f} dist={_dist_resistance/(_sw_atr or 1):.2f}×ATR | RSI={_sw_rsi:.1f} | R:R={_rr:.2f} | vol={_sw_vol/_sw_vol_avg20:.2f}x",
                                         300)
                                    _swing_entry_meta[symbol] = {"tp": _support, "sl": _sl_nat, "r_dist": _sl_dist}
                                    return "entry", f"Swing-Level Resistance (R:R {_rr:.1f})", price
            except Exception as _sw_exc:
                tlog(f"swing_exc:{symbol}", f"[SWING-EXC][SHORT] {symbol}: {_sw_exc}", 300)
        # ─────────────────────────────────────────────────────────────────────
        ...
        if _bl_until > time.time():
            ...
            tlog(f"blacklist:{symbol}", f"[BLACKLIST][SHORT] {symbol} bloccato ancora {_bl_rem:.1f}h", 300)
            return None, None, None
...
def active_loss_trailing_worker_short():
    """
    Active Loss Trailing (ALT) — il ratchet "al contrario" per SHORT.
    Se il trade non mostra momentum positivo (mfe_roi < ALT_CONFIRM_ROI) entro
    le prime ore, stringe progressivamente lo SL verso l'entry:
    ...
    while True:
        try:
            for symbol in list(open_positions):
                ...
                # SHORT: SL è sopra l'entry — stringere = abbassare lo SL verso il prezzo
                new_sl = float(entry_price) + (mult * float(r_dist))
                current_sl = float(entry.get("sl", float('inf')))
                # Il nuovo SL deve essere strettamente più basso di quello corrente
                if new_sl >= current_sl:
                    entry["alt_step"] = target_step
                    set_position(symbol, entry)
                    continue
                # Il nuovo SL deve rimanere sopra il prezzo corrente
                price_now = get_last_price(symbol)
                if not price_now or new_sl <= price_now:
                    continue
                ok = set_position_stoploss_short(symbol, new_sl)
                entry["alt_step"] = target_step
                entry["sl"] = new_sl
                with _state_lock:
                    position_data[symbol] = entry
                tlog(f"alt_short:{symbol}",
                     f"[ALT][SHORT] {symbol} Step{target_step} età={age_h:.1f}h mfe={entry.get('mfe_roi',0.0):.1f}% "
                     f"SL {current_sl:.6f}→{new_sl:.6f} (×{mult:.2f}R) set={ok}", 60)
                notify_telegram(
                    f"⚡ [ALT][SHORT] SL stretto — {symbol}\n"
                    f"Età: {age_h:.1f}h senza conferma momentum\n"
                    f"SL: {current_sl:.6f} → {new_sl:.6f}\n"
                    f"Perdita max ridotta del {(1-mult)*100:.0f}%"
                )
        except Exception as _e:
            log(f"[ALT-SHORT][CRASH] {_e}")
        time.sleep(30)
...
alt_thread_short = threading.Thread(target=active_loss_trailing_worker_short, daemon=True)
alt_thread_short.start()
...
while True:
    ...
    for symbol in eligible_symbols:
        ...
        if signal == "entry":
            ...
            _is_pullback_signal = "Rimbalzo" in (strategy or "") or "RS-Breakdown" in (strategy or "") or "Swing-Level" in (strategy or "")
            ...
            if not qty or qty == 0:
                ...
                continue
            # TP1: per segnali Swing-Level usa il supporto naturale (TP fisso strutturale SHORT);
            ...
            if _is_swing_entry and _swing_meta.get("tp") and _swing_meta.get("sl"):
                tp1_price = float(_swing_meta["tp"])   # supporto strutturale (TP SHORT)
                r_dist    = float(_swing_meta["r_dist"])  # distanza SL swing (più stretta)
                final_sl_swing = float(_swing_meta["sl"])   # SL naturale sopra resistenza
                _hard_cap = MAX_LOSS_CAP_PCT if symbol in VOLATILE_ASSETS else MAX_LOSS_CAP_PCT_STABLE
                final_sl  = min(final_sl_swing, price_now * (1.0 + _hard_cap))
                log(f"[SWING-TP][SHORT] {symbol} TP naturale={tp1_price:.6f} SL={final_sl:.6f} R:R={(price_now-tp1_price)/(final_sl-price_now):.2f}")
            else:
