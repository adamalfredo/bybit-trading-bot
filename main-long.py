...
def analyze_asset(symbol: str):
    ...
    try:
        ...
        if rs_4h is not None:
            _rs_threshold = -1.5 if not _btc_favorable_long else -3.0
            if rs_4h < _rs_threshold:
                tlog(f"rs_filter:{symbol}",
                     f"[RS-FILTER][LONG] {symbol} rs_4h={rs_4h:+.2f}% < soglia {_rs_threshold:+.1f}% → coin troppo debole vs BTC, skip",
                     600)
                return None, None, None
...
while True:
    # ══════════════════════════════════════════════════════════════════
    # PULLBACK ENTRY: controlla ogni ciclo se un pending ha toccato EMA20
    # ══════════════════════════════════════════════════════════════════
    for _pb_sym in list(pending_pullbacks.keys()):
        try:
            ...
            if time.time() > _pb["expiry_ts"]:
                ...
                continue
            # 2. Segnale invalidato? (RSI < 45 = momentum perso)
            ...
            if _pb_rsi < 45 or _pb_macd_val < _pb_macd_sig:
                del pending_pullbacks[_pb_sym]
                tlog(f"pb_inv:{_pb_sym}", f"[PULLBACK] {_pb_sym} segnale invalidato (RSI={_pb_rsi:.0f}, MACD bearish), cancellato", 60)
                continue
            # 3. Il prezzo ha toccato la zona EMA20?
            _pb_price = get_last_price(_pb_sym)
            if not _pb_price:
                continue
            _pb_ema20 = _pb["ema20"]
            _pb_zone   = _pb_ema20 * (1 + PULLBACK_ZONE_PCT)
            if _pb_price <= _pb_zone:
                # Prezzo tornato a EMA20: possiamo entrare
                # Ma prima verifica che tutti i gate siano ancora ok
                if _pb_sym in open_positions or len(open_positions) >= MAX_OPEN_POSITIONS:
                    del pending_pullbacks[_pb_sym]
                    continue
                if ENTRY_PAUSED or not _btc_daily_regime_ok or _btc_dumping_long or _weekly_block:
                    del pending_pullbacks[_pb_sym]
                    continue
                _pb_r_dist = _pb["r_dist"]
                _pb_atr    = _pb["atr"]
                _pb_strategy = _pb.get("strategy", "Pullback EMA20")
                log(f"[PULLBACK-TRIGGER] {_pb_sym} prezzo={_pb_price:.4f} ≤ zona EMA20={_pb_zone:.4f} → ENTRY")
                del pending_pullbacks[_pb_sym]
                # Esegui entry come market (al prezzo corrente ≈ EMA20)
                ...
                if not qty_str_pb:
                    continue
                qty_pb = market_long(_pb_sym, order_amount_pb, qty_exact=qty_str_pb)
                if not qty_pb or qty_pb == 0:
                    continue
                _entry_price_pb = get_last_price(_pb_sym) or _pb_price
                _sl_pb = max(_entry_price_pb - _pb_r_dist, _entry_price_pb * 0.95)
                _tp1_pb = _entry_price_pb + TP1_R * _pb_r_dist
                _qty_tp1_pb = qty_pb * TP1_PARTIAL
                ok_pos_sl_pb = set_position_stoploss_long(_pb_sym, _sl_pb)
                if not ok_pos_sl_pb:
                    place_conditional_sl_long(_pb_sym, _sl_pb, qty_pb, trigger_by="MarkPrice")
                tp_oid_pb = None
                if _qty_tp1_pb > 0:
                    ok_tp_pb, tp_oid_pb = place_takeprofit_long(_pb_sym, _tp1_pb, _qty_tp1_pb)
                set_position(_pb_sym, {
                    ...
                    "trailing_active": False, "p_max": _entry_price_pb,
                    "r_dist": _pb_r_dist, "breakout_exempt": False,
                })
                add_open(_pb_sym)
                _daily_trades_opened += 1
                _trade_log("entry", _pb_sym, "LONG", entry_price=_entry_price_pb, qty=qty_pb,
                           sl=_sl_pb, tp=_tp1_pb, r_dist=_pb_r_dist, extra={"pullback": True})
                notify_telegram(f"🟢📈 LONG aperto {_pb_sym} (PULLBACK EMA20)\nPrezzo: {_entry_price_pb:.4f}\nStrategia: {_pb_strategy}\nSL: {_sl_pb:.4f}\nTP1: {_tp1_pb:.4f}")
        except Exception as _pb_exc:
            tlog(f"pb_exc:{_pb_sym}", f"[PULLBACK-EXC] {_pb_sym}: {_pb_exc}", 300)
    # ══════════════════════════════════════════════════════════════════
    ...
    for symbol in eligible_symbols:
        ...
        if signal == "entry":
            ...
            # Non entrare subito al segnale (= top del movimento).
            ...
            if not _is_pullback_signal:
                ...
                if _ema20_val and _ema20_val > 0:
                    _pb_zone = _ema20_val * (1 + PULLBACK_ZONE_PCT)
                    _already_at_ema20 = price_now_calc <= _pb_zone
                    if not _already_at_ema20:
                        # Metti in pending solo se non già presente (evita sovrascrittura)
    ...
    for symbol in list(open_positions):
        ...
        if price_now >= entry_price:
            continue
        # Verifica se il segnale tecnico della coin si è invertito (exit confermato)
        try:
        ...
        if not qty_live or qty_live <= 0:
            continue
        tlog(f"signal_expiry:{symbol}", f"[SIGNAL-EXPIRY][LONG] {symbol} zombie {age_h:.1f}h, segnale invertito ({reason}) → chiudo", 300)
